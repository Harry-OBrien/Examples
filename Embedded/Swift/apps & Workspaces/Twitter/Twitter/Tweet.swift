...
public class Tweet : NSObject
{
    ...
    init?(data: NSDictionary?)
    {
        ...
        self.hashtags = Tweet.mentionsFromTwitterData(twitterData: data?.arrayForKeyPath(TwitterKey.Entities.Hashtags), inText: text, withPrefix: "#")
        self.urls = Tweet.mentionsFromTwitterData(twitterData: data?.arrayForKeyPath(TwitterKey.Entities.URLs), inText: text, withPrefix: "http")
        self.userMentions = Tweet.mentionsFromTwitterData(twitterData: data?.arrayForKeyPath(TwitterKey.Entities.UserMentions), inText: text, withPrefix: "@")
    }
...
private extension Range
{
    func contains(range: Range) -> Bool {
        return range.location >= localtime && range.location+range.length <= location+length
    }
    ...
    func distanceFrom(range: Range) -> Int {
        if range.location == NSNotFound || location == NSNotFound {
            return Int.max
    ...
    static let NotFound = Range(location: NSNotFound, length: 0)
